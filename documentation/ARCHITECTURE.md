...
│  - routing orchestration (cache + Valhalla + detour)   │
│  - hazard push enqueue (flag → Cloud Tasks)        │
│  - in-instance LRU cache (cacheManager)            │
└────────────┬────────────────────────────────┬──────┘
             │                                │
             │ (sync Valhalla resolve + detour) │ (async, on blocking transition)
             ▼                                ▼
┌────────────┬──────────────┐   ┌─────────────┬──────────────────────────┐
│   Routing Engine (Valhalla) │   │  Hazard Push Pipeline                  │
...
Express.js + TypeScript on a single `onRequest` export (`api`, region `asia-southeast1`, `memory: '512MiB'`, `timeoutSeconds: 120`, `maxInstances: 20`). Project: `roadassistapp-c2e37`. The routing engine is a separate Cloud Run service (self-hosted Valhalla, scale-to-zero, `--max-instances=2`); the hazard push pipeline (Cloud Tasks + FCM) is documented in [PIPELINE.md](./PIPELINE.md). Cloud layout and deploy flow live in [INFRASTRUCTURE.md](./INFRASTRUCTURE.md) and [DEPLOY.md](./DEPLOY.md).
...
- Business logic: existence checks, consensus/TTL rules, passability scoring, Valhalla orchestration, data shaping.
...
- **Routing**: width maps to buckets (`<0.8` NARROW, `≤1.0` MEDIUM, else WIDE, default MEDIUM); the cache key is origin/stops/dest rounded to 5 decimals plus bucket plus costing (stop-less keys keep the `origin:dest:bucket:costing` form; pre-costing keys re-solve once and self-heal). Misses call self-hosted Valhalla on Cloud Run (`VALHALLA_URL`, provisioned by `infra/valhalla/setup.sh`, `motor_scooter` costing — or `auto` for car-class `vehicleType`, both built-in — no custom profile needed) and persist the GeoJSON geometry (as a JSON string — Firestore rejects nested arrays; parsed back on cache hits) plus ETA and a 30-day `expiresAt` enforced in code. `POST /routes` accepts up to 10 ordered `stops`, solved as one Valhalla call with `locations: [origin, …stops, dest]` and cached under a stops-aware key. When `width` is provided, the resolved route is additionally gated on our own alley data: any `alley_segments` entry narrower than the vehicle within 20 m of the route joins the avoidance set — the engine has no width-class `exclude`; this gate handles app-measured widths natively. Every request — hit or miss — is re-validated against active blocking flags of every hazard type (`FLOOD`, `OBSTRUCTION`, `ACCIDENT`) via `closureService.findBlocking` (Turf `lineIntersect` + vertex-containment hit test in `utils/geo`; distance/sorting keep the existing planar math). On a hit the service re-solves closure-aware through Valhalla with all blocking circles (hazards + width pseudo-zones) passed as `exclude_polygons` — each failed attempt merges newly crossed zones into the exclusion set (up to 4 attempts reusing true reported radii) — and returns `200 {source: "detour", hazards}`; detours are never cached, and anything still blocked returns soft-blocked with `hazards` instead of `409`, so no cache invalidation is ever needed (details in [CACHE.md](./CACHE.md#hazard-feedback-loop)). Valhalla fetches time out after 15 s with one retry, so a scale-to-zero engine's cold boot reads as one slow request instead of a 500. Engine behavior deltas and limits (OSRM vs Valhalla) are tabulated in [ENGINE.md](./ENGINE.md). Route options (2026-09): the service returns `{cached, routes}` with up to 5 options — stop-less requests use Valhalla `alternates: 4` in the same call (image pins `max_alternates: 6`), stop requests return 1; a blocked primary falls back to a safe alternative, blocked alternatives get their own detour attempt, and anything still blocked returns soft-blocked (raw geometry + `hazards`) instead of `409` — `409` survives only for width-only blocks and an origin/stop/destination inside a flag zone (named `EndpointBlockedError`); block decisions log as `[routing] …`; the cache stores the routes array with a legacy single-route read.
- **Hazard feedback loop (decision record)**: evaluated 2026-09, migrated 2026-09. Blocking moved inside the router: Valhalla `exclude_polygons` carries every blocking circle (hazards + width pseudo-zones) per request, so detours grow natively around closures instead of via-probed waypoints (the old OSRM stitch stack — `detour`/`detourProbe`/`hasUTurnSpike`/`viaTangentReversal` — is deleted). Hazard-blocked requests now return soft-blocked (`200` + raw geometry + `hazards`) instead of `409`, so a route is always shown when the engine has one and no cache invalidation is ever needed; `409` survives only for width-only blocks (physical impossibility). Hazards reuse the flag pipeline (all three types + `"2"`/`"3"`, per-type TTL decay, admin lock) instead of a new domain; impact radii default per type (`FLOOD` 200 m, `OBSTRUCTION`/`ACCIDENT` 100 m, overridable per flag via `radiusMeters`); unflag is creator-retract hard-delete (`"3"` protected) so both flag expiry and unflag unblock routes on the next request.
...
- **Routing engine economics (decision record)**: evaluated 2026-09, migrated 2026-09. The engine runs scale-to-zero (no `min-instances`) — cost-per-request ≈ $0.5–1.5/mo at trial scale instead of ~$190/mo always-on (4Gi/2vCPU, sized to fit the Vietnam tiles in RAM; the Valhalla image is ~2.2 GB vs ~4.9 GB for the old OSRM one). Cold starts are absorbed by two layers: the durable 30-day `routing_cache` means the engine is only touched on cache misses (OD pairs saturate fast at low user counts), and the single Valhalla retry covers the residual cold-boot race. Prod recommendation: `ROUTING_CACHE_TTL_SECONDS=7776000` (90 d) to stretch cache warmth. Valhalla won over GraphHopper as the native-avoidance engine (`exclude_polygons` per request, built-in `motor_scooter` costing replacing the custom Lua profile, zero license/data cost); the earlier OSRM stitch workaround and its spike-rejection heuristics are deleted with the migration.
